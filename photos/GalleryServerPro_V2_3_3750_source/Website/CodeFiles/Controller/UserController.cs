...
namespace GalleryServerPro.Web.Controller
{
	...
	public static class UserController
	{
		...
		/// This function is intended to be called periodically; for example, once during each application restart. Since it iterates through
		/// all albums, it is resource intensive and should be involked on a separate low-priority thread.
		/// </summary>
		public static void DeleteOrphanedAlbumOwners()
		{
			System.Data.DataTable users = GetUserNames();
			List<String> userNames = new List<string>(users.Rows.Count);

			foreach (System.Data.DataRow user in GetUserNames().Rows)
			{
				userNames.Add(user[0].ToString());
			}

			DeleteOrphanedAlbumOwner(Factory.LoadRootAlbumInstance(), userNames);
		}
