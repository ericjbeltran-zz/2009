...
namespace GalleryServerPro.Web.gs.pages.admin
{
	public partial class usersettings : Pages.AdminPage
	{
		...
		private void ConfigureOrphanUserAlbums()
		{
			...
			if (!Core.EnableUserAlbum) // Do not use CoreConfig.enableUserAlbum: During a postback where a user enables user albums, CoreConfig.enableUserAlbum
			{													// returns true, but GetUserAlbum() will not return valid album IDs because it returns int.MinValue when Core.EnableUserAlbum=false
			...
			{
				string userAlbumParentTitle = Util.RemoveHtmlTags(Factory.LoadAlbumInstance(CoreConfig.userAlbumParentAlbumId, false).Title);
		...
		private List<IAlbum> GetOrphanUserAlbums()
		{
			...
			foreach (UserEntity user in UserController.GetAllUsers())
			{
				int userAlbumId = UserController.GetUserAlbumId(user.UserName);
			...
			int albumId = CoreConfig.userAlbumParentAlbumId;
